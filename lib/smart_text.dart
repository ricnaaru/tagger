...
class SmartText extends StatelessWidget {
  ...
  String _compute(double maxWidth) {
    ...
    while (found > -1 && found <= this.data.length) {
      ...
      String ellipsized = _getEllipsizedText(
        temp,
        this.style,
        maxWidth,
        withDot: lineCounter == _maxLines - 1 && withDot,
        wholeWord: (lineCounter + 1) >= _maxLines ? false : wrapWholeWord,
      );
...
String _getEllipsizedText(
  ...
  if (wholeWidth <= (maxWidth - 2)) return text;
  ...
  if (wholeWord) {
    ...
    String nextText = text.substring(i, i + 1);
